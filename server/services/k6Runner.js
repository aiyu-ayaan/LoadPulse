...
const toLiveMetrics = (state) => {
  ...
  return {
    ...
    avgLatencyMs,
    p95LatencyMs: round(percentile(state.durationSamples, 95)),
    p99LatencyMs: round(percentile(state.durationSamples, 99)),
    errorRatePct,
    throughputRps,
  ...
  await TestRun.updateOne(
    { _id: state.runId },
    {
      $set: {
        ...
        "liveMetrics.p95LatencyMs": liveMetrics.p95LatencyMs,
        "liveMetrics.p99LatencyMs": liveMetrics.p99LatencyMs,
